...
public class TeamOptimizationService : ITeamOptimizationService
{
    ...
    {
        // Default to 36 hours if no schedule exists
        var defaultHours = 36;

        try
        {
            var schedule = await _context.WorkSchedules
                .Where(ws => ws.EmployeeId == employeeId && ws.EffectiveTo == null)
                .FirstOrDefaultAsync();
            ...
            var allocatedHours = currentAllocations.Sum(pa =>
                (int)(pa.AllocationPercentage / 100.0 * schedule.StandardHoursPerWeek));

            return Math.Max(0, schedule.GetAvailableHours() - allocatedHours);
        }
        catch
        {
